...
# TCP连接保活策略，默认设置为300秒，则server端会每300秒向连接空闲的客户端发起一次ACK请求， 以检查客户端是否已经挂掉，对于无响应的客户端则会关闭其连接
# 如果设置为0，则不会进行保活检测
tcp-keepalive 300
...
# maxclients 10000
############################## MEMORY MANAGEMENT ################################
# 设置最大可用内存，单位是字节，默认被注释，表示不限制，如果启用rdb功能，这个值应该设置在物理内存的45%以下，当rdb发生dump时发生oom
# maxmemory <bytes>
